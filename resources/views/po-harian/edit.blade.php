...
<main class="h-full overflow-y-auto">
    <div class="container px-6 mx-auto grid">
        ...
        <div class="grid grid-cols-1 gap-3">
            <!-- Dokumen -->
            <form
                action="{{route('bd-harian-dok.update', $data->id)}}"
                method="POST"
                id="storeDok"
            ...
            >
                ...
                <label class="block mt-4 text-sm">
                    ...
                    <select class="block shadow-sm border p-2 rounded-md w-full mt-1 text-sm dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 form-multiselect focus:border-stone-400 focus:outline-none focus:shadow-outline-stone dark:focus:shadow-outline-gray" name="id_tiket" id="id_tiket">
                        @foreach($dok_tiket as $it)
                            <option value="{{$it->id}}" {{old('id', $data->id) == $nu->id ? 'selected' : ''}}>{{$it->id}} - {{$it->nom_unit}}</option>
                        @endforeach
                    </select>
                </label>
                ...
                <label class="block mt-4 text-sm">
                    ...
                    <input value="{{old('uraian', $data->uraian)}}" class="block shadow-sm border p-2 rounded-md w-full mt-1 text-sm dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 form-multiselect focus:border-stone-400 focus:outline-none focus:shadow-outline-stone dark:focus:shadow-outline-gray" type="text" name="uraian" id="uraian">
                </label>
